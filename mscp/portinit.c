...
void mscp_istep1( mscpa_t *a ) {
    ...
    if ( ~sa_in & 0x8000 ) {
        printf("MSCP: Step 1 did not have high bit set. Restarting\n");
        return;
    }
    ...
    printf("MSCP Init sequence step 1: Vector = %04o, C-Ring size: %i, R-Ring size: %i, IE=%i, WR=%i\n",
            a->vector, a->csize, a->rsize, a->init_ie, a->diag_wr );

    if ( a->diag_wr ) {
        printf("MSCP: Going to debug wraparound mode until reset\n");
...
void mscp_istep2( mscpa_t *a ) {
    ...
    if ( a->init_ie && a->vector != 0 )
        status = lesi_send_intr( a->vector, sa_out );
    else
        status = lesi_sa_write( sa_out );
    if ( status )
        goto err;
    ...
    printf("MSCP Init sequence step 2: Ringbase = %07o, PI=%i\n",
...
void mscp_istep3( mscpa_t *a ) {
    ...
    printf("MSCP Init sequence step 3: Ringbase = %09o, PP=%i\n",
...
void mscp_istep4( mscpa_t *a ) {
    ...
    if ( a->step == 4 ) {
        ...
        csize = 1 << a->csize;
        rsize = 1 << a->rsize;
        a->cahdr_base = a->ringbase - sizeof(mscp_cahdr_t);
        ...
        printf("DMA start!\n");
        status = lesi_write_dma_zeros( csize * 2 + rsize * 2 + sizeof(mscp_cahdr_t) / 2 );
        if ( status ) {
            printf("DMA failed!\n");
    ...
    if ( a->step == 4 ) {
        ...
        printf( "MSCP Init sequence step 4: Burst = %i, GO=%i LF=%i\n",
                a->burst, go, lf );
        if ( a->burst == 0 ) {
            a->burst = MSCP_DEF_BURSTSZ;
            printf("MSCP Init sequence step 4: Default burst size of %i requested\n", a->burst);
        }
    } else {
        printf( "MSCP Init wait: GO=%i\n", go );
    }
    ...
    if ( go ) {
        ...
        a->rring_idx = 0;
        a->cring_idx = 0;
        a->cmd_queue = NULL; // TODO: free old queue entries!
        a->c_poll = 0;
        a->csize = 1 << a->csize;
        a->rsize = 1 << a->rsize;
        a->step = STEP_READY;
...
void mscp_diagpp( mscpa_t *a ) {
    ...
    if ( a->features & FEAT_PURGE ) {
        ...
        printf("MSCP Purge poll test got PURGE\n");
    } else {
        printf("MSCP Purge poll skipping purge test as the KLESI does not support it\n");
    }
    ...
    printf("MSCP Purge poll test got POLL\n");
